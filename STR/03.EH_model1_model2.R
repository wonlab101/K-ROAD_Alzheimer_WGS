...
####### 1. Model 1 Length Test ####### 
{
  ...
  write.table(dat.test, "outputs/eh_RCL40_visual_single_str_test_inv_noPC_pca_rm_APOE_with_orig.txt", row.names=F, col.names = T, sep="\t", quote=F) 
...
####### 2.1.  Model 2 Threshold Test  ####### 
{
  ...
  write.table(dat.list, "outputs/eh_RCL40_visual_single_str_thr_test_ref_PC_rm.txt", row.names=F, col.names = T, sep="\t", quote=F)
...
####### 2.2. Model 2 Threshold Test with frequency preparing   ####### 
{
  ...
  ### data preprocessing
  {
    ...
    data.table::fwrite(dat.t.test.s.ref_df, "eh.v5_RCL40_visual_allele_long_ref_pivot_longer.txt.gz", compress = "gzip")
    dat.t.test.s.ref_df = data.table::fread('eh.v5_RCL40_visual_allele_long_ref_pivot_longer.txt.gz')
...
####### 1. Model 1 Length Test ####### 
{
  ...
  write.table(dat.test, "outputs/eh_DX_single_str_test_inv_noPC_pca_rm_APOE.txt", row.names=F, col.names = T, sep="\t", quote=F) 
...
####### 2.1. Model 2 Threshold Test  ####### 
{
  ...
  write.table(dat.list, "outputs/eh_DX_single_str_thr_test_ref_PC_rm.txt", row.names=F, col.names = T, sep="\t", quote=F) 
...
####### 2.2. Model 2 Threshold Test with frequency preparing ####### 
{
  ...
  write.table(dat.list, 'outputs/eh_DX_single_str_thr_freq_ref_PC_rm.txt', row.names=F, col.names = T, sep="\t", quote=F) #Write output file
  ...
  ### data preprocessing
  {
    ...
    data.table::fwrite(dat.t.test.s.ref_df, "eh.v5_DX_allele_long_ref_pivot_longer.txt.gz", compress = "gzip")
    dat.t.test.s.ref_df = data.table::fread('eh_DX_allele_long_ref_pivot_longer.txt.gz')
    dat.t.test.s.ref_df <- dat.t.test.s.ref_df %>% filter(sample %in% pheno$sample)
