...
####### 1. Model 1 Length Test ####### 
{
      ...
      tryCatch(
        {
          ...
          allele_count_inv_norm_glm = coef(summary(glm(is_case ~ allele_count_inv_norm + is_female + batch + age + depth + s_avg_depth +
                                                         PC1 + PC2 + PC3, data = dat.temp, family = "binomial")))
          dat.test[i,]$allele_count_inv_norm_logistic_p<-allele_count_inv_norm_glm[2,4]
          dat.test[i,]$allele_count_inv_norm_effect<-allele_count_inv_norm_glm[2,1]
  ...
  write.table(dat.test, "outputs/eh_RCL40_visual_single_str_test_inv_noPC_240419_pca_rm.txt", row.names=F, col.names = T, sep="\t", quote=F) 
...
# Input file prepare
{
  ...
  # Phenotype data
  pheno = read_tsv("~/Dropbox/SMC_AD_WGS_paper/Data/WGS_1824_phenotype_update.240416.txt") %>% dplyr::rename(sample = IID)
  pheno = pheno %>% select(sample,RCL40_visual, batch ,age,sex, DX,PC1,PC2,PC3 )
  pheno = pheno %>% filter(!sample %in% pca_rm_samples)
  pheno = pheno %>% filter(sample != 'WGS_1622')
...
####### 1. Model 1 Length Test ####### 
{
  ...
  for(i in 1:length(str_list)){
    ...
    if(T){
      tryCatch(
        {
          allele_count_glm = coef(summary(glm(is_case ~ allele_count + is_female + batch +
                                                PC1 + PC2 + PC3 +
                                                age + depth + s_avg_depth, data = dat.temp, family = "binomial")))
  ...
  write.table(dat.test, "outputs/eh_RCL40_visual_single_str_test_inv_notestPC_noPC_240419_pca_rm.txt", row.names=F, col.names = T, sep="\t", quote=F) # Write output file
