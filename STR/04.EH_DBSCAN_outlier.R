...
# dat[1:5,1550:1558]
dat<-left_join(dat.annot,dat, by = c('index' = 'index'))
dat[1:5,1:10]
dat %>% dim()
dat[1:5,1515:1521]
...
write_tsv(dat.test,"outputs/eh_DBSCAN_outlier.tsv")

dat.dbscan<-read.delim(paste0("outputs/eh_DBSCAN_outlier.tsv"), stringsAsFactors = F, sep="\t", header=T)
