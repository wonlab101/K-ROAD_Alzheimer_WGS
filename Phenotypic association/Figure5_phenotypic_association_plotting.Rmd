---
title: "Fig5.Diverse effects of genetic variants on Aβ levels and cognitive function."
output: Fig5
---
...
rm(list=ls())
sessionInfo()
setwd("/Users/minyoungcho/Desktop/SKKU/Projects_SKKU/WGS_AD/00.Paper_writing/plot_250131/")
DIR_output <- "/Users/minyoungcho/Desktop/SKKU/Projects_SKKU/WGS_AD/00.Paper_writing/plot_250131/"
...
### 1) import data
```{r}
fn="/Users/minyoungcho/Desktop/SKKU/Projects_SKKU/WGS_AD/heritability/250131_heritability_calcualtion_rmNA_RV_SV.txt"
...
### 3) regression analysis - Amyloid beta
#model1
```{r}
...
#data
```{r}
fn="/Users/minyoungcho/Desktop/SKKU/Projects_SKKU/WGS_AD/prs/WGS_1824_phenotype_update.250131.txt"
...
result_df$P_value <- ifelse(result_df$"Pr(>|t|)" < 0.05, "Significant", "Non-significant")

# Define the list of target Dependent_Variable values
target_variables <- c("RCL global")
...
p5.3.1 <- ggplot(filtered_result_df, aes(x = Estimate, y = Dependent_Variable, xmin = Estimate - 1.96*`Std. Error`, xmax = Estimate + 1.96*`Std. Error`, color = P_value)) +
  geom_point(size = 3) +
...
#"MMSE", "CDRSB", "Verbal_memory", "Visual_memory",
result_df <- data.frame()
...
result_df$P_value <- ifelse(result_df$"Pr(>|z|)" < 0.05, "Significant", "Non-significant")
...
```{r}
p5.4.1 <- ggplot(result_df, aes(x = APOE_RNV_SV_Group, y = Estimate, 
...
## merging plots
```{r, fig.width=30, fig.hight=20}

row1.1 <- ggarrange(p5.0.1, p5.0.2, labels = c('a',''),
                  ncol = 2, font.label = list(size = 28), label.y = 1.01)
print(row1.1)

##legend
legend_5.1 <- get_legend(p5.1.1 + theme(legend.position="bottom", legend.direction = "horizontal"),)

row1.2 <- ggarrange(p5.1.1, p5.1.2, p5.1.3, p5.1.4, p5.1.5, p5.1.6, legend_5.1,labels = c('b','','','','','',''),
                  nrow = 7, font.label = list(size = 28), label.y = 1.01)

# Print the arranged plots
print(row1.2)

row1.3 <- ggarrange(p5.3.1.2, p5.3.2,  nrow = 2,  labels = c('d','e'), font.label = list(size = 28), heights = c(1,2),label.y = 1.01, common.legend = TRUE, legend = "bottom")
print(row1.3)


row1 <- ggarrange(row1.1, row1.2,row1.3, nrow = 3,heights = c(1,2.5,2.5) )

print(row1)

row2 <- ggarrange(p5.2.1, p5.2.2, p5.2.3, p5.2.4, p5.2.5, p5.4.1, nrow = 6, heights =  c(1,1,1,1,1.8,1.8),
                  labels = c('c','','','','','f'),font.label = list(size = 28), label.y = 1.01)
print(row2)



#combined_plot_abcd <- ggarrange(row1, row2,row3,  ncol = 3,widths = c(1.5,1,1,1) )
combined_plot <- ggarrange(row1, row2, font.label = list(size = 28), ncol = 2, heights = c(3,1.2))
print(combined_plot)


#combined_plot <- ggarrange(combine_1, row3, nrow = 2,heights = c(1.8,1) )
#print(combined_plot)



ggsave(paste0(DIR_output,'Fig5_ver9_RNVonly_250131.pdf'),combined_plot, device = "pdf", width = 30, height = 25)
